        ...
        <div className="max-w-3xl">
          ...
          <div className="mt-9 max-w-2xl">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs font-medium text-muted">
              ...
              <span>{hero.hint2}</span>
              <span className="hidden h-1 w-1 rounded-full bg-border-strong sm:block" aria-hidden />
              <span>{hero.hint3}</span>
            </div>
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link href="/graph" className="inline-flex h-11 items-center rounded-md border border-border-strong bg-surface px-5 text-sm font-semibold text-foreground transition-[border-color,color,transform] hover:border-accent hover:text-accent active:scale-[0.98]">
              {hero.narratorNetwork}
            </Link>
          </div>
        </div>

        <aside className="research-folio min-w-0" aria-label={hero.folioRecord}>
          <div className="flex items-center justify-between gap-4 border-b border-[color:var(--folio-line)] px-5 py-4 sm:px-7">
            <div>
              <p className="text-xs font-semibold text-[color:var(--folio-muted)]">{hero.folioRecord}</p>
              <p className="mt-1 font-mono text-xs font-semibold text-[color:var(--folio-accent)]" dir="ltr">AK-1-42</p>
            </div>
            <p dir="rtl" lang="ar" className={`${amiri.className} text-2xl text-[color:var(--folio-ink)]`}>{hero.folioKitab}</p>
          </div>

          <div className="px-5 py-6 sm:px-7 sm:py-7">
            <p className="text-xs font-semibold text-[color:var(--folio-muted)]">{hero.folioChainLabel}</p>
            ...
            <div className="mt-6 grid grid-cols-[1fr_auto] items-end gap-5 border-t border-[color:var(--folio-line)] pt-5">
              <div>
                <p className="text-xs font-semibold text-[color:var(--folio-muted)]">{hero.folioSource}</p>
                <p className="mt-1 text-sm font-semibold text-[color:var(--folio-ink)]">{hero.folioSourceValue}</p>
              </div>
              <span className="source-seal" aria-label={hero.folioVerified}>
